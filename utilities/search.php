...
$query1 = $con->prepare("SELECT * FROM albums WHERE UPPER(title) LIKE UPPER(:keyword)");
$query1->bindParam(":keyword", $search);
$query1->execute();
$sqldata1 = $query1->fetchAll(PDO::FETCH_ASSOC);
$albums = "";
$activeAlbumResult = " activeAlbumResult";
$musics = "";
$tracks = "[";
$trackIterator = 0;
foreach($sqldata1 as $row1)
{
    $albumCover = $row1["cover"];
    $albumID = $row1["id"];
    $albums .= "<div class='app-card$activeAlbumResult' id='searchAlbum$albumID'>
                    <img src='$albumCover'/>
                </div>";
